package Test::GenOO::NGS::Tag;
use strict;

use base qw(Test::GenOO::Locus);
use Test::More;
...
sub sample_object {
	return Test::TestObjects->get_testobject_GenOO_NGS_Tag;
}
sub data {
	return {
		species => {
...
sub test_objects {
	...
	my @test_loci = @{Test::GenOO::Locus->test_objects()};
	
	for (my $i=0;$i<12;$i++) {
		my $test_locus = $test_loci[$i];
		push @test_objects, $test_class->class->new({
			STRAND => $test_locus->strand,
			CHR    => $test_locus->chr,
			START  => $test_locus->start,
			STOP   => $test_locus->stop,
			SCORE  => $i + 1
		});
